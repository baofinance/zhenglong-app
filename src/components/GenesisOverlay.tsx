...
export default function GenesisOverlay({
  ...
  return (
    <div className={`relative ${className}`}>
      {children}
      {isActive && (
        <div className="absolute inset-0 bg-black/15 flex items-center justify-center cursor-not-allowed backdrop-blur-sm">
          <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 bg-black/80 backdrop-blur-md py-6 border-y border-[#4A7C59]/20">
            <div className="text-center space-y-4 max-w-md mx-auto px-8">
              ...
              <Link
                href="/genesis"
                className={`inline-block px-4 py-2 bg-[#4A7C59] hover:bg-[#3A6147] text-white text-sm font-medium transition-colors ${geoClassName}`}
              >
                Genesis Page
              </Link>
            </div>
